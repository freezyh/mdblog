...
let offsetTopList: any = [];

function setOffsetTopList() {
...
onMounted(() => {
  setOffsetTopList();
  parent.value = document.querySelector("#__nuxt") as HTMLElement;
  parent.value?.addEventListener("scroll", scrollHandle);
  window.onresize = () => {
    setOffsetTopList();
  };
});
...
onUnmounted(() => {
  parent.value?.removeEventListener("scroll", scrollHandle);
});
</script>

<template>
  <div class="sidebar-container" :class="{ show: sidebarStore.anchor }">
    <a href="javascript:void(0)" class="sidebar-guide"><i class="icon-spr icon-spr-guide" /></a>
    <div class="sidebar-bd">
      <div class="sidebar-anchor">
        <!-- 锚点定位 回到顶部 -->
        <ul class="sidebar-anchor-list">
          <li
            v-for="(item, index) in sidebarStore.anchorList"
            :key="index"
          ...
          >
            <a class="sidebar-anchor-item-link">
              <i class="sidebaricon" :class="[item.iconClass]" />
          ...
          <li class="sidebar-anchor-item" @click="toTop">
            <a class="sidebar-anchor-item-link" href="javascript:void(0)">
              <i class="sidebaricon sidebaricon-top" />
      ...
      <div class="sidebar-normal qrcode">
        <a href="javascript:void(0)" class="sidebar-normal-btn"><i class="sidebaricon sidebaricon-qrcode" /></a>
        <div class="sidebar-normal-popover">
          <img
            ...
            alt=""
          >
          <p class="sidebar-normal-popover-text">
            关注扣叮公众号
          </p>
        </div>
      </div>
      <div class="sidebar-normal feedback">
        <a href="https://wj.qq.com/s2/4331679/38d8/" class="sidebar-normal-btn" target="_blank" rel="noopener noreferrer">
          <i class="sidebaricon sidebaricon-feedback" />
        </a>
      </div>
    </div>
  </div>
  ...
  @import "@/assets/styles/font/sidebar-fonticon.css";
.sidebar-container {
  position: fixed;
  width: 110px;
  bottom: 80px;
